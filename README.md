...
**Fast Neural Style (arbitrary/mosaic style transfer)** — Real-time neural style transfer that repaints a frame in a learned artistic style.
...
- look development
- music-video / title treatments

## Roles (host-neutral routing)

This plugin contributes the role(s): `style`. A host routes the model by role — it carries no
`targetApps`, so it is not tied to any single application.

**Example host — [Guillotine](https://github.com/HereLiesAz/Guillotine):** Desktop `effect_style` — `apply_image_effect effect=style`.

## Model file(s)

- **`style.onnx`** (role `style`) — [upstream](https://huggingface.co/onnx-community/fast-neural-style-mosaic/resolve/main/mosaic.onnx)

Model license: **BSD-3-Clause (fast-neural-style, ONNX Model Zoo)**. This plugin's manifest/packaging is `BSD-3-Clause`.
...
```sh
npm install && npm run build     # packs com.hereliesaz.azphalt.fast-neural-style-1.0.0.azp
